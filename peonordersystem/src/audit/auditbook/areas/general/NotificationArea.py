...
"""
from peonordersystem.src.MenuItem import is_discount_item
from .abc.GeneralArea import GeneralArea


class NotificationArea(GeneralArea):
    """NotificationArea represents the area
    ...
    def _write_item_data(self, menu_item):
        ...
        self._write_item_data_note(menu_item.notes)
    ...
    def _write_item_data_note(self, note):
        """

        @param note:
        @return:
        """
        format = self._get_item_data_note_format()
        self.write_data(note, col=self.col + 1, format=format)

    def _get_item_data_note_format(self):
        """
